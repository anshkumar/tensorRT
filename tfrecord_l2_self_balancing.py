...
json_path = BASE_PATH+"potato_l1_l2_old_18_19_20_gid.json"
out_path = BASE_PATH+"out"
writer = tf.python_io.TFRecordWriter(BASE_PATH+"potato_l1_l2_old_18_19_20_gid.record")
...
l1_mask = False # L1 in grid will be cropped masked images
...
category_l1 = ['potato']
category_l2 = ['big crack and cuts', 'decayed', 'sprouted', 'major hole', 'shriveled', 'greening', 'small crack and cuts', "badly deformed", "eyes", "confusing", "dried sprout", "small_crack_and_cuts", "badly_deformed", "big_crack_and_cuts", "major_hole", "peels", "dried_sprout"]
    ...
    if data[key]["filename"] not in ignore_filename:
        for j in range(len(data[key]["regions"])):
            if "parent_id" in data[key]["regions"][j]["region_attributes"] and data[key]["regions"][j]["region_attributes"]["parent_id"] in hierarchy_dict:
                ...
                pts = np.asarray([[int((x - node.x1)*node.scale2 + node.x2), int((y - node.y1)*node.scale2 + node.y2)] for x,y in zip(x_lst, y_lst)])
